...
def main(args):
    ...
    if way_D1 == "Lasso":
        output_str += f"-l {best_lambda_D1}\t"
    elif way_D1 == "BSP":
        output_str += f"-rbsp {selected_fv_filename_D1}\t"
    elif way_D1 == "ANN":
        arch_str = ""
        for arc in arch_D1:
            arch_str += f"{arc} "
        output_str += f"-ann {selected_fv_filename_D1} {rho_D1} {maxitr_D1} {arch_str}\t"
    elif way_D1 == "RF":
        params_str = ""
        for (params_name, value) in params_D1.items():
            params_str += f"{params_name} {value} "
        output_str += f"-rf {selected_fv_filename_D1} {params_str}\t"
    elif way_D1 == "ALR":
        output_str += f"-alr {best_lambda_D1}\t"
    elif way_D1 == "DT":
        params_str = ""
        for (params_name, value) in params_D1.items():
            params_str += f"{params_name} {value} "
        output_str += f"-dt {selected_fv_filename_D1} {params_str}\t"  
