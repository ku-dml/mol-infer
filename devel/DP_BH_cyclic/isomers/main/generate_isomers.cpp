...
int main(int argc, char** argv) {
  ...
  if (argc < 9) {
    ...
    cout << "6. an integer of global number of path in DAG (0 if no limit)"
         << endl;
    cout << "7. an integer of local number of path in DAG (0 if no limit)"
         << endl;
    cout << "8. an integer of number of wanted graphs" << endl;
    cout << "9. a file name of output" << endl;
    cout << "10. a file with information of partition (optional now)" << endl;
    exit(-1);
  }
  globalTimeKeeper tk;
  ...
  local_path_limit = stoi(argv[7]);
  num_limit = stoi(argv[8]);
  ...
  if (no_partition_info) {
    ...
    get_partition(base_vertices, base_edges, chLB_v, chUB_v, chLB_e, chUB_e,
                  fixed_v, fixed_e, partitionfilename);
  }
  ...
  for (size_t ind_comp = 0; ind_comp < num_len; ++ind_comp) {
    if (ind_comp < set_tree_component.size()) {
      ...
      if (cp.no_feasible_pair) {
        ...
        unordered_map<DAG_node, DAG_count> DAG_W3_count;
        ...
        if (ch > 0) {
          ...
          GenW1(IN_INFO, all_rs_W1, comp_graph.map_FT_W1, map_W1,
                comp_graph.All_FT_W1, all_trie_node);
          GenW2_0(IN_INFO, all_rs_W2, comp_graph.map_FT_W2, map_W2, map_W2_ind,
                  comp_graph.All_FT_W2, all_trie_node);
          GenW3_0(IN_INFO, all_rs_W3, comp_graph.map_FT_W3, map_W3,
                  comp_graph.All_FT_W3, all_trie_node, IN_INFO.target_rs1.color,
                  IN_INFO.delta_1 + 1);
        ...
        if (ch > 0) {
          ...
          check_tree_component(IN_INFO, all_rs_W3, map_W3, all_trie_node,
                               all_fp_W, ch, IN_INFO.target_rs1);  // IDO
          ...
          Gen_Path_W_in_T_count(map_DAG_W3, comp_graph.map_FT_W1,
                                comp_graph.map_FT_W2, comp_graph.map_FT_W3, ch,
                                DAG_W3_count, path_num, num_tmp);

          double time_tmp_5_2 = globalTimeKeeper::tt.diff();
          DAG_traverse_num_time += time_tmp_5_2 - time_tmp_5_1;
          ...
          if (_EXP) {
            ...
            cout << "time to get exact number of paths and graphs from DAG = "
                 << DAG_traverse_num_time << endl;
            cout << "# paths = " << path_num << endl;
            cout << "# graphs (v-comp) = " << num_tmp << endl;
          }

          size_t local_path_num = 0;
          ...
          for (auto& fp : all_fp_W) {
            ...
            Gen_Path_W_in_T(map_DAG_W3, comp_graph.map_FT_W1,
                            ...
                            all_seq_W3, global_path_num, local_path_num,
                            DAG_W3_count);
    ...
    if (ind_comp < set_edge_component.size()) {
      ...
      if (cp.no_feasible_pair) {
        ...
        unordered_map<DAG_node_T, size_t>
            DAG_T_node_path_num_core;  // each node in DAG has #path by IDO
        ...
        if (chub > 0) {
          ...
          GenW2_0(IN_INFO, all_rs_W2, comp_graph.map_FT_W2, map_W2, map_W2_ind,
                  comp_graph.All_FT_W2, all_trie_node);
          GenW3_0(IN_INFO, all_rs_W3, comp_graph.map_FT_W3, map_W3,
                  comp_graph.All_FT_W3, all_trie_node, M, 3);
        ...
        GenW2_0_core(IN_INFO, all_rs_W2_core, comp_graph.map_FT_W2_core,
                     map_W2_core, comp_graph.All_FT_W2_core, all_trie_node, 2);
        ...
        // Gen_Path_T_path_core_count(map_DAG_T, DAG_T_node_path_num_core);

        // size_t path_num_core = 0;

        // for (auto& fp : all_fp_T) {
        //   if (_GLOBAL_TIME_OUT) continue;
        //   path_num_core += DAG_T_node_path_num_core[fp.node1] *
        //                    DAG_T_node_path_num_core[fp.node2];
        // }
        ...
        Gen_Path_W_count(IN_INFO, all_trie_node, map_DAG_W, DAG_W_root_node,
                         comp_graph.map_FT_W1, comp_graph.map_FT_W2,
                         comp_graph.map_FT_W3, DAG_W_count);

        // Gen_Path_W_path_count(
        //     IN_INFO, map_DAG_W, all_trie_node, chlb, DAG_W_node_path_num,
        //     DAG_W_node_path_num_as_frequency_vector, DAG_W_root_node);

        // Gen_Path_T_path_count(IN_INFO, map_DAG_T, all_trie_node_path,
        //                       DAG_W_node_path_num_as_frequency_vector,
        //                       DAG_T_node_path_num);

        // for (auto& fp : all_fp_T) {
        //   if (_GLOBAL_TIME_OUT) continue;
        //   path_num +=
        //       DAG_T_node_path_num[fp.node1] * DAG_T_node_path_num[fp.node2];
        // }

        // double time_tmp_9_2 = globalTimeKeeper::tt.diff();
        // DAG_traverse_path_time += time_tmp_9_2 - time_tmp_9_3;

        // cout << "# paths of DAG of both core and non-core part = " <<
        // path_num
        //      << endl;
        // cout << "time to traverse all paths from DAG of both core and
        // non-core "
        //         "part = "
        ...
        // Gen_Path_W_graphs_count(IN_INFO, all_trie_node, map_DAG_W,
        //                         DAG_W_root_node, comp_graph.map_FT_W1,
        //                         comp_graph.map_FT_W2, comp_graph.map_FT_W3,
        //                         DAG_W_node_graph_num);

        // Gen_Path_T_graphs_count(IN_INFO, map_DAG_T, all_trie_node_path,
        //                         map_T2_W3, comp_graph.map_FT_W2_core,
        //                         DAG_W_node_graph_num, DAG_T_node_graph_num);

        Gen_Path_T_count(IN_INFO, all_trie_node_path, map_DAG_T, map_T2_W3,
                         comp_graph.map_FT_W2_core, DAG_W_count, DAG_T_count);

        for (auto& fp : all_fp_T) {
          if (_GLOBAL_TIME_OUT) continue;
          path_num += DAG_T_count[fp.node1].path * DAG_T_count[fp.node2].path;

          graph_num +=
              DAG_T_count[fp.node1].graph * DAG_T_count[fp.node2].graph;
        }

        double time_tmp_9_4 = globalTimeKeeper::tt.diff();
        DAG_traverse_num_time += time_tmp_9_4 - time_tmp_9_3;

        if (_EXP) {
          cout << "# feasible pairs = " << feasible_pairs_sum << endl;
          cout << "time for feasible pairs = " << feasible_pairs_time << endl;
          cout << "size of DAG of core part, |V| = " << map_DAG_T.size()
               << endl;
          cout << "size of DAG of core part, |E| = " << DAG_size_e_T << endl;
          cout << "size of DAG of non-core part, |V| = " << map_DAG_W.size()
               << endl;
          cout << "size of DAG of non-core part, |E| = " << DAG_size_e_W
               << endl;
          ...
          cout << "# paths of DAG of both core and non-core part = " << path_num
               << endl;
          cout << "# graphs (e-comp) = " << graph_num << endl;
          cout << "time to get exact number of paths and graphs from DAG = "
               << DAG_traverse_num_time << endl;
        }
        ...
        for (auto& fp : all_fp_T) {
          ...
          Gen_Path_T(map_DAG_T, map_DAG_W, map_T2_W3, comp_graph.map_FT_W1,
                     ...
                     global_path_num, local_path_num, DAG_W_count, DAG_T_count);
          Gen_Path_T(map_DAG_T, map_DAG_W, map_T2_W3, comp_graph.map_FT_W1,
                     comp_graph.map_FT_W2, comp_graph.map_FT_W3,
