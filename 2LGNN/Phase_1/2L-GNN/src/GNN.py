...
def learn_GNN_selfmade_trainvaltest(
    ...
    extra_x_length=None, batch_size=32, num_epoch=300,
    ...
    log_prefix = f"./log/{output_prefix}_{datetime.datetime.today()}"
    ...
    if model_name == "GNN_int":
        ...
        model = network.GNN_2L_no_edge_type(
            ...
            num_layers_FT=16,
            p_channels_FT=256,
            ANN_channels_FT=(256, 128, 64, 32, 16, 8),
            in_channels_IN=num_atom_features,
            hidden_channels_IN=hidden_size,
            num_layers_IN=num_layers,
            p_channels_IN=p_size,
        ...
        ft_dataset_used = True
    elif model_name == "GNN_2L_no_edge_type_GraphSAGE":
        model = network.GNN_2L_no_edge_type_GraphSAGE(
            in_channels_FT=ft_dataset.num_node_features,
            hidden_channels_FT=256,
            num_layers_FT=32,
            p_channels_FT=256,
    ...
    with open(f"./model/{output_prefix}_config.txt", 'w') as f:
        f.write(f"in_channels_FT, {ft_dataset.num_node_features}\n")
        f.write(f"hidden_channels_FT, {256}\n")
        f.write(f"num_layers_FT, {16}\n")
        f.write(f"p_channels_FT, {256}\n")
        f.write(f"ANN_channels_FT, 256, 128, 64, 32, 16, 8\n")
        f.write(f"in_channels_IN, {num_atom_features}\n")
        f.write(f"hidden_channels_IN, {hidden_size}\n")
        f.write(f"num_layers_IN, {num_layers}\n")
        f.write(f"p_channels_IN, {p_size}\n")
        ANN_channels_tmp = [str(ttt) for ttt in ANN_channels]
        ANN_str = ", ".join(ANN_channels_tmp) 
        f.write(f"ANN_channels_IN, {ANN_str}\n")
    ...
    if save is True:
        output_model_name = f"./model/{output_prefix}.pth"
        print(f"The model will be saved as {output_model_name}")
    else:
        output_model_name = None
    early_stopping = network.EarlyStopping(patience=200, verbose=False, path=output_model_name)
    ...
    total_refresh_time = 3 # tmp value

    for _ in range(total_refresh_time):
        ...
        for epoch in range(num_epoch):
            ...
            if best_val_loss is None or val_loss_mae <= best_val_loss:
                ...
                # with open(f"{log_prefix}_testset.txt", 'w') as f:
                #     f.write("val_true, val_pred\n")
                #     for _true, _pred in zip(test_true, test_pred):
                #         f.write(f"{_true}, {_pred}\n")
                output_str = f'Epoch: {epoch+1:04d}, Train MAE: {running_loss_mae:.7f}, Val MAE: {val_loss_mae:.7f}, Val R2: {val_loss_r2:.7f}, Test MAE: {test_loss_mae:.7f}, Test R2: {test_loss_r2:.7f}, Time: {time.time() - time_epoch_start:.7f}.'
                print(output_str)
...
def eval_GNN(train_dataset, val_dataset, test_dataset, ft_dataset, extra_x_length, best_params, num_epoch=300, output_prefix="GNN", resume_filename=None):
    ...
    best_val_loss, test_loss_mae, test_loss_r2 = learn_GNN_selfmade_trainvaltest(train_dataset, val_dataset, test_dataset, ft_dataset, best_params, 
        extra_x_length=extra_x_length, random_seed=RANDOM_SEED, save=True, num_epoch=num_epoch, output_prefix=output_prefix, resume_filename=resume_filename)
