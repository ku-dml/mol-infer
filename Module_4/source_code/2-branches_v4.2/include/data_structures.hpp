...
bool operator==(const half_path_status& lhs, const half_path_status& rhs){
    return (
        // lhs.len == rhs.len && 
        lhs.lrs == rhs.lrs && lhs.rrs == rhs.rrs);
}
//template <class T>
//Azam//
using Color = int;
...
namespace std{
    ...
    public:
        size_t operator()(const half_path_status& aa) const {
            // size_t res = std::hash<size_t>{} (aa.len);
            size_t res = std::hash<root_status>{}(aa.lrs);
            // hash_combine<root_status>(res, aa.lrs);
    ...
    public:
        size_t operator()(const map_rv& aa) const {
            ...
            // size_t res = std::hash <vector <size_t>>{} (aa.w_ind);
...
void read_input(const string& infilename, input_info& IN_INFO) {
    ...
    IN_INFO.k1 = (IN_INFO.diameter - 5) / 2;
    IN_INFO.k2 = IN_INFO.diameter - 5 - IN_INFO.k1;
    ...
    void calc_bc(const input_info& IN_INFO,
        ...
        for (size_t i = start_index; i < m; ++i){
            if (seq[i].first != 0){
                ...
                // cout  << "i = " <<  i <<  " d1 = " << d1 << " d2  =  " << d2  << " mul = " << mul << endl;
                if (d1 <= d2){
...
// A trie structure to store the trie
class PathTrie{
public:
    unsigned short key;  
    size_t parent_ind; // parent index
    vector <int> children;
