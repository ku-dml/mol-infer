######
#7/19/2021
# To implement the grid search procedure with ANN
##################################

from twolayered_MILP_2LMM_L import *
from read_instance_2layer_2LMM_L import *
import ann_inverter, lr_inverter
...
def main(argv):
    ...
    # prop = "./MILP_result/At/At_3elem"
    # target_value_lb = -1000000
    # target_value_ub = 1000000
    # instance_file = "./MILP_result/At/instance_b1_2LMM.txt"
    # fringe_tree_file = "./MILP_result/At/ins_b1_fringe_2LMM.txt"
    # output_prefix = "test_0727"
    # p_max_file = "p_max_delta_r.txt"
    # THETA = ["./Kow/Kow", "./Lp/Lp", "./Sl/Sl"]
    ...
    p_max_file = argv[7]
    THETA = argv[8:] # list of names
    
    ## extending Vecdelta, Vecr
    p_max, Vecdelta, Vecr = read_pmax_file(p_max_file)

    ### preprocess to prepare Lambda and setting names of files ###
    start = time.time()
    ...
    #  #input file of fv
    ann_training_all_data_filename = "{}_desc.csv".format(prop)
    ann_training_data_filename = "{}_selected_desc.csv".format(prop)
    ann_training_data_norm_filename = "{}_selected_desc_norm.csv".format(prop)
    ...
    ### read dataset for standardization
    training_data = ann_inverter.read_training_data(
        ann_training_data_norm_filename)
    # dataset = dt_inverter.read_training_data(normalized_dataset_filename)
    # des = dt_inverter.read_fv_descriptors(normalized_dataset_filename)
    ...
    ### create MILP object
    MILP = pulp.LpProblem(name="MILP_2LMM_ANN_GS")
    
    
    ## reading from the seed graph ##
    V_C, E_C, \
    ...
    Lambda, Lambda_dg_int, Lambda_int, Gamma_int, Gamma_int_ac = prepare_lambda_pre( 
        ann_training_all_data_filename, Lambda, Lambda_dg_int, Lambda_int, Gamma_int, Gamma_int_ac
    )
    ...
    milp_ann_vars = ann_inverter.initialize_lp_variables(
        ann, ann_a, ann_b, forbidden_node, "def")
    ...
    x_hat, _ = prepare_variables_nor_std_fv(num_fv)
    ...
    THETA_descriptors = dict()
    THETA_num_fv = dict()
    THETA_mass_ind = dict()
    THETA_forbidden_node = dict()
    THETA_I_integer = dict()
    THETA_I_nonneg = dict()
    THETA_max_dcp = dict()
    THETA_min_dcp = dict()
    THETA_avg_dcp = dict()
    ...
    for theta in THETA:
        ...
        if ".txt" in theta: # user defined weights not using LLR
            ...
            # file for normalized csv
            normalized_dataset_filename = "{}_desc_norm.csv".format(theta)
            # file for fringe trees
            fv_fringe_tree_filename = "{}_fringe.txt".format(theta)    # all fringe trees used in learning
        fv_fringe_tree, index_set = read_fringe_tree(fv_fringe_tree_filename, strF)
    
        THETA_descriptors[theta], THETA_num_fv[theta], THETA_mass_ind[theta],\
        THETA_max_dcp[theta], THETA_min_dcp[theta], THETA_avg_dcp[theta], THETA_sd_dcp[theta], \
        THETA_forbidden_node[theta], THETA_I_integer[theta], THETA_I_nonneg[theta] = prepare_fv(
            original_dataset_filename, Lambda_int, Lambda_ex, Gamma_int, Gamma_int_less, Gamma_int_equal,
            Gamma_lf_ac, fv_fringe_tree,
            index_set, n_G, n_G_int, MASS, dg, dg_int, bd_int, na_int, na_ex, ec_int, fc, ac_lf, rank_G
        )
        THETA_x_hat[theta], _ = prepare_variables_nor_std_fv(THETA_num_fv[theta], f"{theta_counter}")
        MILP = add_constraints_nor_std_fv(
            MILP,
            THETA_num_fv[theta], THETA_mass_ind[theta], THETA_descriptors[theta], mass_n,
            THETA_max_dcp[theta],THETA_min_dcp[theta], THETA_avg_dcp[theta], THETA_sd_dcp[theta],
            THETA_x_hat[theta],
            std_eps,
        ...
        fp = open(LR_filename)
        THETA_LR[theta] = lr_inverter.read_LR(fp)

        THETA_LR[theta].build_weight_var(THETA_I_integer[theta], THETA_I_nonneg[theta], f"{theta_counter}")

        fp.close()
        MILP = add_constraints__LR(MILP, THETA_x_hat[theta], THETA_num_fv[theta],
            THETA_LR[theta], THETA_mass_ind[theta], mass_n, THETA_forbidden_node[theta], f"{theta_counter}")
    ...
    if pulp.LpStatus[MILP.status] == "Optimal":
        ...
        while flag:
            try:
                ...
                # y_predict = ann_inverter.inspection(ann_training_data_norm_filename, f"{test_prefix}_desc_norm.csv", ann, x_hat, std_eps)
                flag = False
            except:
                pass
        ...
        for z in Z_sort:
            ...
            # MILPp = copy.deepcopy(MILP)
            MILPp = MILP
            for i in range(len(THETA)):
                THETA_LR[THETA[i]].build_constraints(MILPp, ALPHA_star[THETA[i]] + (z[i] - 0.5) * Vecdelta[i], 
                                                     ALPHA_star[THETA[i]] + (z[i] + 0.5) * Vecdelta[i], f"{i+1}")
