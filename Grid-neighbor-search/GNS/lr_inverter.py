...
class LinearReg:
    ...
    def build_weight_var(self, I_integer, I_nonneg, prop = "def"):
    ...
    def build_constraints(self, MILP, y_lb, y_ub, prop = "def"):
        if f"LR_lr_{prop}" in MILP.constraints:
            MILP.constraints[f"LR_lr_{prop}"] = self.predict[0] == pulp.lpSum([self.coef[i - 1] * self.weight_var[i]
                                                    for i in range(1, self.K + 1)]) + self.bias
        else:
            MILP += self.predict[0] == pulp.lpSum([self.coef[i - 1] * self.weight_var[i]
                                               for i in range(1, self.K + 1)]) + self.bias, f"LR_lr_{prop}"
        if f"LR_ub_{prop}" in MILP.constraints:
            MILP.constraints[f"LR_ub_{prop}"] = self.predict[0] <= y_ub
        else:
            MILP += self.predict[0] <= y_ub, f"LR_ub_{prop}"

        if f"LR_lb_{prop}" in MILP.constraints:
            MILP.constraints[f"LR_lb_{prop}"] = self.predict[0] >= y_lb
        else:
            MILP += self.predict[0] >= y_lb, f"LR_lb_{prop}"
    ...
    def _predict_alpha_star(self, des):
        y = 0
        # if len(des) != self.K:
        #     print("""
        #     Error predicting the value.
        ...
        #     sys.exit()
        # else:
        y = sum([self.coef[i] * des[i + 1].value() for i in range(0, self.K)])
        y += self.bias
