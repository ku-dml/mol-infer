...
from src import LR, ALR, CLSF, FUN
...
def BSP(x, y, arr_D, best_D, best_train, best_median, best_min, log_filename, Sbar=FUN.Status_Bar()):
    ...
    return tmp_D, best_D, best_train, best_median, best_min, tmp_median, tmp_min, Sbar

def BSP_main(x, y, best_train, best_median, best_min, log_filename, Sbar=FUN.Status_Bar()):
    x_size = x.shape[1]
    ...
    for _ in range(x_size - 1):
        ...
        arr_D, best_D, best_train, best_median, best_min, tmp_median, tmp_min, Sbar = \
            BSP(x, y, arr_D, best_D, best_train, best_median, best_min, log_filename, Sbar=Sbar)
        time_end = time.time()
    ...
    return best_D, best_train, best_median, best_min, Sbar

def LLR_ALR_pre(x, y_org, fv_list, log_filename, metric='R2', Sbar=FUN.Status_Bar()):
    ...
    Sbar_pre = FUN.Status_Bar()
    Sbar_pre.set_prop_name(Sbar.prop_name + "_pre")
    Sbar_pre.verbose = Sbar.verbose
    Sbar_pre.set_total_times(len(lambda_list) * Times_pre_1 * Fold)
    Sbar_pre.output()
    ...
    for lmd in lambda_list:
        ...
        if len(selected_fv) == 0:
            TsR[lmd] = [-1000000000]
            TrR[lmd] = [-1000000000]
            Sbar_pre.sub_total_times(Times_pre_1 * Fold)
            Sbar_pre.output()
            continue
        ...
        for split_seed in range(1, Times_pre_1+1):
            ...
            for train, test in kf.split(x_sel):
                ...
                _, r2train, r2test = LR.learn_MLR(x_sel[train], y[train], x_sel[test], y[test], metric=metric)
                ...
                Sbar_pre.append(time_end - time_start)
    ...
    for lmd in lambda_list_2:
        ...
        _ect += (len(selected_fv) * (len(selected_fv) + 1) / 2 - 1 + 1) * Times_pre_2 * Fold
    Sbar.set_total_times(_ect)
    Sbar.output()
    ...
    for lmd in lambda_list_2:
        ...
        tmp_D, tmp_train, tmp_median, tmp_min, Sbar = BSP_main(x_sel, y, tmp_train, tmp_median, tmp_min, log_filename, Sbar=Sbar)
