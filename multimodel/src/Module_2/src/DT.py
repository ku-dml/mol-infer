...
def learn_decision_tree(
    x_train, y_train, x_test, y_test, params=DecisionTreeRegressor().get_params(), metric='R2'
):
    if metric == 'R2':
        ...
        dt.set_params(**params)
        dt.set_params(class_weight="balanced")
        dt.fit(x_train, y_train)
    ...
    if metric == 'R2':
        ...
        r2train, r2test = CLSF.calc_bacc(dt, x_train, y_train, x_test, y_test)
    elif metric == 'AUCROC':
        r2train, r2test = CLSF.calc_aucroc(dt, x_train, y_train, x_test, y_test)
        ...
        for values in itertools.product(*map(SEARCH_PARAMS.get, keys)):
            ...
            for split_seed in range(1, Times_pre+1):
                ...
                for train, test in kf.split(x_selected):
                    ...
                    r2train, r2test, _, _ = learn_decision_tree(x_selected[train], y[train], x_selected[test], y[test], params=params, metric=metric)
                    time_end = time.time()
