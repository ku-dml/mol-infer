######
#   2023/01/01   modified from 2LMM_q, to 2LMM_SEP
#   2023/01/09   for GNS
######

from twolayered_MILP_2LMM_L import *
from read_instance_2layer_2LMM_L import *
import lr_q_inverter, ann_inverter

import pulp_modified as pulp
# import pulp

import os, subprocess, sys, argparse, itertools, copy
...
def get_args():
    ...
    parser.add_argument("p_max_file", help='p_max_file')

    parser.add_argument('-d1', '--d1', nargs='+', type=str)
    parser.add_argument('-d2', '--d2', nargs='+', type=str)
    parser.add_argument('-tp', '--thetap', nargs='*', type=str)
    parser.add_argument('-t', '--theta', nargs='+', type=str)

    args = parser.parse_args()

    return args

def add_separating_plane(
    MILP, w_sep, b_sep, select_D1, normalized_dataset_filename,
    Lambda_int, Lambda_ex, Gamma_int, Gamma_int_less, Gamma_int_equal, Gamma_lf_ac, fv_fringe_tree,
    ...
    x_hat_sp, x_tilde_sp = prepare_variables_nor_std_fv(K_l, "sp")
    ...
    if select_D1:
        MILP += pulp.lpSum(x_hat_sp[i] * w_sep[i - 1] for i in range(1, K_l + 1)) + b_sep <= 0, "separating_plane"
    else:
        MILP += pulp.lpSum(x_hat_sp[i] * w_sep[i - 1] for i in range(1, K_l + 1)) + b_sep >= 0, "separating_plane"

    return MILP, x_hat_sp, x_tilde_sp
...
def main(args):
    ...
    p_max_file = args.p_max_file
    if args.thetap is not None:
        THETA_p = int(args.thetap[0])
    else:
        THETA_p = 6
    THETA = args.theta
    ...
    # file for original csv, var0
    original_dataset_filename = "{}_desc.csv".format(prop)
    # file for normalized csv
    normalized_dataset_filename = "{}_desc_norm.csv".format(prop) # used for separating plane
    selected_dataset_filename_D1 = "{}_D1_desc_norm_selected.csv".format(prop)
    selected_dataset_filename_D2 = "{}_D2_desc_norm_selected.csv".format(prop)
    ...
    value_filename_D1 = "{}_D1_values.txt".format(prop)
    value_filename_D2 = "{}_D2_values.txt".format(prop)

    # file for separating plane
    sep_filename = "{}_sep.txt".format(prop)
    # read the separating plane information
    a_tilde, w_sep, b_sep = read_separating_plane_file(sep_filename) 
    ...
    # check to use D1 or D2
    if target_value_ub < a_tilde:
        select_D1 = True
        str_D = "D1"
        selected_way = way_D1
    elif target_value_lb > a_tilde:
        select_D1 = False
        str_D = "D2"
        selected_way = way_D2
    else:
        ...
        if random.random() >= 0.5:
            select_D1 = True
            str_D = "D1"
            target_value_ub = a_tilde
            selected_way = way_D1
        else:
            select_D1 = False
            str_D = "D2"
            target_value_lb = a_tilde
            selected_way = way_D2
    ...
    #####
    # this part should be revised as choosing one of D1 or D2, with a constraint with separating plane
    if select_D1:
        descriptors_l, descriptors_list, descriptors_l_list, descriptors_q_x, descriptors_q_y, descriptors_q_x_minus, descriptors_q_y_minus, \
            K_l, K_q, K_l_ind, K_q_ind, mass_ind, forbidden_node = \
                prepare_fv(
                    selected_dataset_filename_D1, Lambda_int, Lambda_ex, Gamma_int, Gamma_int_less, Gamma_int_equal, Gamma_lf_ac, fv_fringe_tree,
                    index_set, n_G, n_G_int, MASS, dg, dg_int, bd_int, na_int, na_ex, ec_int, fc, ac_lf, rank_G
                )
    else:
        descriptors_l, descriptors_list, descriptors_l_list, descriptors_q_x, descriptors_q_y, descriptors_q_x_minus, descriptors_q_y_minus, \
            K_l, K_q, K_l_ind, K_q_ind, mass_ind, forbidden_node = \
                prepare_fv(
                    selected_dataset_filename_D2, Lambda_int, Lambda_ex, Gamma_int, Gamma_int_less, Gamma_int_equal, Gamma_lf_ac, fv_fringe_tree,
                    index_set, n_G, n_G_int, MASS, dg, dg_int, bd_int, na_int, na_ex, ec_int, fc, ac_lf, rank_G
                )
    ...
    # separating plane
    MILP, x_hat_sp, x_tilde_sp = add_separating_plane(
        MILP, 
        w_sep, b_sep, select_D1, normalized_dataset_filename,
        Lambda_int, Lambda_ex, Gamma_int, Gamma_int_less, Gamma_int_equal, Gamma_lf_ac, fv_fringe_tree,
        index_set, n_G, n_G_int, MASS, dg, dg_int, bd_int, na_int, na_ex, ec_int, fc, ac_lf, rank_G,
        max_dcp, min_dcp, avg_dcp, sd_dcp,
        mass_n
    ...
    if select_D1:
        y_min_2, y_max_2 = read_dataset(selected_dataset_filename_D1, value_filename_D1)
    else:
        y_min_2, y_max_2 = read_dataset(selected_dataset_filename_D2, value_filename_D2)
    target_value_lb = (target_value_lb - y_min_2) / (y_max_2 - y_min_2)
    target_value_ub = (target_value_ub - y_min_2) / (y_max_2 - y_min_2)

    # need to revise this part for ANN
    ### read LR file to obtain constants
    if selected_way == "R-MLR":

        LR_filename = f"{prop}_{str_D}_linreg.txt"
        LR_filename_org = LR_filename[:]
        fp = open(LR_filename)
    ...
    elif selected_way == "ANN":
        #input file of bias
        ann_bias_filename = "{}_{}_biases.txt".format(prop, str_D)
        #input file of weight
        ann_weights_filename = "{}_{}_weights.txt".format(prop, str_D)
        if select_D1:
            ann_training_data_norm_filename = selected_dataset_filename_D1
        else:
            ann_training_data_norm_filename = selected_dataset_filename_D2
...
#### The next part is to implement grid search procedure ### 
    ...
    for theta_counter, theta in enumerate(THETA, 1):
    ...
    if pulp.LpStatus[MILP.status] == "Optimal":
        y_star = y.value()
        y_star_scaled = (y_star * (y_max_2 - y_min_2) + y_min_2) * (y_max - y_min) + y_min
    ...
    if pulp.LpStatus[MILP.status] == "Optimal":
        ...
        #         # need to modify, with support with quad_desc and ANN
        #         if selected_way == "R-MLR":
        #             y_predict, x_hat_vector = lr_q_inverter.inspection(f"{test_prefix}_desc_norm.csv", LR_filename_org, x_hat, std_eps,
        ...
        #         # print("Inspection value (scaled):  ", (y_predict * (y_max_2 - y_min_2) + y_min_2) * (y_max - y_min) + y_min)
        #         flag = False
        #     except:
        #         pass
        ...
        with open(output_prefix + "_GS_log.csv", "w") as f:
            ...
            f.write(f"Original, {output_status}, {solve_end - init_end}, {y_star_scaled}, {(y_predict * (y_max_2 - y_min_2) + y_min_2) * (y_max - y_min) + y_min}, -- \n")
            f.close()
        ...
        for z_counter, z in enumerate(Z_sort, 1):
            print(z)
            ...
            if pulp.LpStatus[MILPp.status] == "Optimal":
                y_star = y.value()
                y_star_scaled = (y_star * (y_max_2 - y_min_2) + y_min_2) * (y_max - y_min) + y_min
            if CPLEX_TIMELIMIT != 0 and solve_end - solve_end_MILPp > CPLEX_TIMELIMIT:
            ...
            if pulp.LpStatus[MILPp.status] == "Optimal":
                ...
                #         # need to modify, with support with quad_desc and ANN
                #         if selected_way == "R-MLR":
                #             y_predict, fv_MILPp = lr_q_inverter.inspection(f"{test_prefix}_desc_norm.csv", LR_filename_org, x_hat, std_eps,
                #                 K_l, K_q, K_l_ind, K_q_ind,
                #                 descriptors_list, descriptors_l_list, descriptors_q_x, descriptors_q_y, descriptors_q_x_minus, descriptors_q_y_minus)
                #             y_predict = y_predict[0]
                #         elif selected_way == "ANN":
                #             y_predict, fv_MILPp = ann_inverter.inspection(f"{test_prefix}_desc_norm.csv", ann_training_data_norm_filename, ann, x_hat, std_eps)
                #         # print("Inspection value:  ",  y_predict)
                #         # print("Inspection value (scaled):  ", (y_predict * (y_max_2 - y_min_2) + y_min_2) * (y_max - y_min) + y_min)
                #         flag = False
                #     except:
                ...
                with open(output_prefix + "_GS_log.csv", "a") as f:
                    f.write(f"{z_counter}, {output_status}, {solve_end - solve_end_MILPp} , {y_star_scaled}, {(y_predict * (y_max_2 - y_min_2) + y_min_2) * (y_max - y_min) + y_min},")
...
if __name__ == '__main__':
    ...
    # ins_list = ["a", "b1", "b2", "b3", "b4", "c", "d"]
    # # ins_list = ["c", "d"]
    # tv_list = [(-1000, 1000), (-1000, 1000),(-1000, 1000),(-1000, 1000),(-1000, 1000),(-1000, 1000),(-1000, 1000)]

    # ins_list = ["a"]
    # # ins_list = ["c", "d"]
    # tv_list = [(-1000, 1000)]

    # for (ins, (tv_lb, tv_ub)) in zip(ins_list, tv_list):
    #     main((0, "./MILP_result/Bp/Bp", tv_lb, tv_ub, f"./MILP_result/Bp/instance_{ins}_2LMM.txt", f"./MILP_result/Bp/ins_{ins}_fringe_2LMM.txt", f"test_{ins}_0816"))
    #     # main((0, "./Homo/Homo", -100000, 100000, f"./Homo/instance_{ins}_2LMM.txt", f"./Homo/ins_{ins}_fringe_2LMM.txt",
    #     #       f"test_{ins}_0110"))