/* 2021/4/30
Generate isomers where fringe trees with H are given and the input
graph can have multivalence and charged atoms.
Input:
- A chemical graphs that can hvae multivalence and charged atoms,
- a list of fringe trees with H and
- a partition file
These are the output of 2LM-M (2L model for multivalence)
Output:
Isomers of the given chemical graph.

New feature: The output graphs will have exactly the same freqs
of fringe trees as that of the input graph. 6/16/2021

Warning: The input graph should be H-Unsuppressed

****************
Old version
DP_2LM-H (DP for 2L model with H )
...
int main(int argc, char** argv) {
  ...
  if (argc < 11) {
    ...
    cout << "6. an integer of global number of paths in DAG" << endl;
    cout << "7. an integer of number of wanted isomers" << endl;
  ...
  get_atom_set_from_TS(g_atom_set, TS_map);
  // for(auto& t:g_atom_set)cout<<t.NAME<<" ";cout<<endl;
  ...
  for (size_t i = 0; i < base_vertices.size(); ++i) {
    ...
    get_descriptors_tree(_cp.IN_INFO, g, h, core_set, internal_set, v, g_atom_set, TS_map);
    ...
    if (fixed_v[i]) {
      ...
      _cp.no_feasible_pair_loose = true;
    }
  ...
  for (size_t i = 0; i < base_edges.size(); ++i) {
    ...
    get_descriptors_edge(_cp.IN_INFO, g, h, core_set, internal_set, e, g_atom_set, TS_map);
  ...
  for (size_t ind_comp = 0; ind_comp < num_len; ++ind_comp) {
    if (ind_comp < set_tree_component.size()) { // v-component
      ...
      if (cp.no_feasible_pair) {
        ...
        if (_DEBUG)
          cout << "ind_comp = " << ind_comp <<  " ch = " << ch << " IN_INFO.height = " << IN_INFO.height
               << " v = " << IN_INFO.base_vertices[0]
               << " delta_i = " << IN_INFO.delta_1 << " d = " << IN_INFO.target_rs1.deg << " m = " << IN_INFO.target_rs1.val << endl;
        ...
        if (ch > 0) {
          ...
          merge_W3(IN_INFO, all_rs_W1, all_rs_W3, map_W1, map_W3, all_trie_node,
                   ch, ch, IN_INFO.delta_1, false);  // IDO
          // size_t chl2 = ch - 1 - chl1;
    ...
    if (ind_comp < set_edge_component.size()) {
      ...
      if (cp.no_feasible_pair) {
        ...
        if (_DEBUG)
          cout << "ind_comp = " << ind_comp << " IN_INFO.base_vertices[0] = " << IN_INFO.base_vertices[0]
        ...
        GenT2_0(IN_INFO, all_rs_T2, map_T2, map_T2_W3, map_T2_ind,
                all_trie_node_path, all_rs_W2_core, map_W2_core, all_rs_W3,
                map_W3, all_trie_node, min(IN_INFO.chUB, core_height), false); 
		// change min in the above to IN_INFO.chUB to find all possible 
		// e-components with different height
        ...
        if (chub > 0) {
          ...
          merge_W3_DAG(IN_INFO, all_rs_W1, all_rs_W3, map_W1, map_W3,
                       all_trie_node, map_DAG_W, 1, chub, 2, true);
  ...
  if (!core_height_fixed) {
    Gen_Graph_unfixed_ch(h, core_set, set_tree_component, set_edge_component,
    ...
    Gen_Graph(h, core_set, set_tree_component, set_edge_component, g_n, g_m,
