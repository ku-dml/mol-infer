...
####################################################
def learn_random_forest(x_train, y_train, x_test, y_test, params=RandomForestRegressor().get_params()):
    ...
    return r2train, r2test, randomforest.predict(x_train), randomforest.predict(x_test)
        ...
        for values in itertools.product(*map(SEARCH_PARAMS.get, keys)):
            ...
            for split_seed in range(1, Times_pre+1):
                ...
                for train, test in kf.split(x_selected):
                    ...
                    r2train, r2test, _, _ = learn_random_forest(x_selected[train], y[train], x_selected[test], y[test], params=params)
                    time_end = time.time()
