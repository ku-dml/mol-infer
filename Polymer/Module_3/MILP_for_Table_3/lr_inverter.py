...
class LinearReg:
    ...
    def build_weight_var(self, I_integer, I_nonneg, name):
        self.weight_var = {i: pulp.LpVariable(f"LR_x_{i}_{name}", cat=pulp.LpContinuous) for i in range(1, self.K + 1)}
        self.predict = {0: pulp.LpVariable(f"LR_y_{name}", cat=pulp.LpContinuous)}
    ...
    def build_constraints(self, MILP, y_lb, y_ub, name):
        MILP += self.predict[0] == pulp.lpSum([self.coef[i - 1] * self.weight_var[i] for i in range(1, self.K + 1)]) + self.bias, f"LR_lr_{name}"
        MILP += self.predict[0] <= y_ub, f"LR_ub_{name}"
        MILP += self.predict[0] >= y_lb, f"LR_lb_{name}"
...
def main(argv):
    ...
    lr_filename = argv[4]
    fv_gen_name = "./2LMM_v017/FV_2LMM_V017"
    ...
    print(y)
    print(y[0] * (1.11 + 11.6) - 11.6)

if __name__=="__main__":
    main((0, "Sl.sdf", "Vd_a_16_17.sdf", "Sl_desc_norm.csv", "Sl_linreg.txt"))
