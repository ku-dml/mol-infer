...
class DTRegInv:
    ...
    def build_var(self, decision_tree_pulp_model: DecisionTree):
        self.prediction_val = pulp.LpVariable(
            f"DT_{decision_tree_pulp_model.tree_idx}_p", cat=pulp.LpContinuous)
        # the number of decision_tree_pulp_model.value is equal to that of leaves
        self.delta_list = {idx: pulp.LpVariable(
            f"DT_{decision_tree_pulp_model.tree_idx}_delta_{idx}", cat=pulp.LpBinary) for idx in decision_tree_pulp_model.value.keys()}

    # build_constraints builds constraints for a given decision tree
    def build_constraints(self, MILP, feature_vector, decision_tree_pulp_model: DecisionTree):
        ...
        MILP += pulp.lpDot(deltas,
                           values) == self.prediction_val, f"DT_{decision_tree_pulp_model.tree_idx}_prediction_value"
        # deltas constraint (4)
        MILP += pulp.lpSum(
            self.delta_list.values()) == 1, f"DT_{decision_tree_pulp_model.tree_idx}_one_fv_visits_only_one_leaf"
            ...
            if type(node) == InnerNode:
                ...
                for leaf_node in leaves:
                    MILP += feature_vector[node.feature_idx] <= node.threshold + M*(
                        1-self.delta_list[leaf_node.idx]), f"DT_{decision_tree_pulp_model.tree_idx}_inner_{node.idx}_leaf_{leaf_node.idx}"
                # inner_node constraints for left children leaves. (6)
                leaves = decision_tree_pulp_model.right_leaf_children(node.idx)
                for leaf_node in leaves:
                    MILP += feature_vector[node.feature_idx] >= node.threshold + np.finfo(np.float).eps - M*(
                        1-self.delta_list[leaf_node.idx]), f"DT_{decision_tree_pulp_model.tree_idx}_inner_{node.idx}_leaf_{leaf_node.idx}"
            else:
                continue
        return MILP
...
class RFRegInv:
    ...
    def build_constraints(self, MILP, y_lb, y_ub, feature_vector, random_forest: RandomForst):
        ...
        MILP += self.ensembled_val == pulp.lpSum(prediction_value_list)/len(
            self.dt_reg_invs), "average-of-values-is-equal-to-y"
        for dt_reg_inv, tree in zip(self.dt_reg_invs, random_forest.dt_list):
            # build constraints for each tree. (3), (4), (5), (6)
            MILP = dt_reg_inv.build_constraints(
                MILP, feature_vector, tree)
        return MILP
