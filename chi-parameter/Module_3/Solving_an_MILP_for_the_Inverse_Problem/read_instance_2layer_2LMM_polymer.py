...
def read_seed_graph(filename):
    ...
    n_LB_lnk = int(F.pop(0))
    n_UB_lnk = int(F.pop(0))
    ...
    for i in range(num):
        ...
        tmp_2 = (arr[1], arr[0], int(arr[2]))
        nu_lnk.append(tmp_1)
        if tmp_1 not in Gamma_ac_lnk:
            Gamma_ac_lnk.append(tmp_1)
        if tmp_2 not in Gamma_ac_lnk:
            Gamma_ac_lnk.append(tmp_2)
    ...
    for i in range(num):
        ...
        if tmp_1 not in Gamma_lnk:
            Gamma_lnk.append(tmp_1)
        if tmp_2 not in Gamma_lnk:
            Gamma_lnk.append(tmp_2)
    ...
    for i in range(num):
        s = F.pop(0)
        arr = list(s.split(' '))
        ns_LB_cnt[(arr[0], int(arr[1]))] = int(arr[2])
        ns_UB_cnt[(arr[0], int(arr[1]))] = int(arr[3])
    ...
    for i in range(num):
        ...
        ac_LB_lnk[(a1, a2, m)] = int(arr[1])
        ac_LB_lnk[(a2, a1, m)] = int(arr[1])
        ac_UB_lnk[(a1, a2, m)] = int(arr[2])
        ac_UB_lnk[(a2, a1, m)] = int(arr[2])
    ...
    for i in range(num):
        ...
        ec_LB_lnk[(a1, a2, m)] = int(arr[1])
        ec_LB_lnk[(a2, a1, m)] = int(arr[1])
        ec_UB_lnk[(a1, a2, m)] = int(arr[2])
        ec_UB_lnk[(a2, a1, m)] = int(arr[2])

    # read E_C_lnk #cao
    num_E_C_lnk = int(F.pop(0))
    s = F.pop(0)
    E_C_lnk = list(map(int, s.split(' ')))

    # # read fc_LB and fc_UB #cao
    # s = F.pop(0)
    # num = int(s)
    # fc_LB = {}
    # fc_UB = {}
    ...
    #     s = F.pop(0)
    #     arr = list(map(int, s.split(' ')))
    #     fc_LB[arr[0]] = int(arr[1])
    #     fc_UB[arr[0]] = int(arr[2])
...
def read_solute_fv_file(solute_fv_file):
    fv = pd.read_csv(solute_fv_file, sep=",", index_col=0)
    solute_fv = dict()
    for fv_name in list(fv.columns):
        solute_fv[fv_name] = fv[fv_name][0]
