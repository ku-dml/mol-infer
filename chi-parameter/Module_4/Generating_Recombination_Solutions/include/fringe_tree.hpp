...
void GenW1(
  ...
  for (unsigned short a = 0; a < M; ++a) {
    All_FT_W1[a].clear();
    for (size_t i = 0; i < IN_INFO.fringe_tree_indices.size(); ++i){
      auto& TS_tmp = TS_map.at(IN_INFO.fringe_tree_indices[i]);
      if (IN_INFO.find_index(TS_tmp.delta_2[0]) == a 
        && 
        IN_INFO.fc_map.find(IN_INFO.fringe_tree_indices[i]) != IN_INFO.fc_map.end()    // fc used in this component
      ){
        // check something
        FringeTree FT(IN_INFO, TS_tmp, IN_INFO.dmax);
        FT.calc_root_val(IN_INFO.dmax);
		    get_root_H(TS_tmp, FT.root_val); // adding valence of H
        FT.calc_rv(IN_INFO, 1, IN_INFO.fringe_tree_indices[i]);
        FT.root_degree += 1;

        // cout << "W1 i = "<<i<<" FT.root_degree = "<<FT.root_degree <<" FT.root_val = " << FT.root_val <<endl; 
        
        if (FT.height == 2 && FT.root_val + 1 <= IN_INFO.atoms[a].VALENCE && FT.root_degree <= 4){
          All_FT_W1[a].push_back(FT);
        }
    ...
    for (size_t i = 0; i < All_FT_W1[a].size(); ++i) {
      ...
      if (_DEBUG) {
        cout << "W1 ind = " << ind << " FT.root_charge = " << T.charge[0] << " FT.root_degree = "<<T.root_degree <<" FT.root_val = " << T.root_val << endl;
        print(T, IN_INFO);
      }

      map_FT_W1[make_pair(rs, ind)].push_back(i);
...
void GenW2_0(
  ...
  for (unsigned short a = 0; a < M; ++a) {
    ...
    for (size_t i = 0; i < IN_INFO.fringe_tree_indices.size(); ++i){
      ...
      if (IN_INFO.find_index(TS_tmp.delta_2[0]) == a
        &&
        IN_INFO.fc_map.find(IN_INFO.fringe_tree_indices[i]) != IN_INFO.fc_map.end()
      ){
        FringeTree FT(IN_INFO, TS_tmp, IN_INFO.dmax);
        FT.calc_root_val(IN_INFO.dmax);
		    get_root_H(TS_tmp, FT.root_val); // adding valence of H
        FT.calc_rv(IN_INFO, 2, IN_INFO.fringe_tree_indices[i]);
        FT.root_degree += 2;

        // cout << "W2 i = "<<i<<" FT.root_degree = "<<FT.root_degree <<" FT.root_val = " << FT.root_val <<endl; 
        // print(FT);

        if (FT.root_val + 2 <= IN_INFO.atoms[a].VALENCE && FT.root_degree <= 4){
          All_FT_W2[a].push_back(FT);
        }
      }
    }

	// special case of single vertex for 2L-model
    //if (2 <= IN_INFO.atoms[a].VALENCE){
    //  FringeTree FT(a, IN_INFO.dmax);
    //  FT.calc_root_val(IN_INFO.dmax);
    //  FT.calc_rv(IN_INFO, 2);
    //  FT.root_degree += 2;
    ...
    for (size_t i = 0; i < All_FT_W2[a].size(); ++i) {
      ...
      if (_DEBUG) {
        cout << "W2_0 ind = " << ind << " FT.root_charge = " << T.charge[0] << " FT.root_degree = "<<T.root_degree <<" FT.root_val = " << T.root_val <<endl;
        print(T, IN_INFO);
      }

      map_FT_W2[make_pair(rs, ind)].push_back(i);
  ...
  for (unsigned short a = 0; a < M; ++a) {
    ...
    for (size_t i = 0; i < IN_INFO.fringe_tree_indices.size(); ++i){
      ...
      if (IN_INFO.find_index(TS_tmp.delta_2[0]) == a
        ...
        FringeTree FT(IN_INFO, TS_tmp, IN_INFO.dmax);
        FT.calc_root_val(IN_INFO.dmax);
        FT.calc_rv(IN_INFO, delta_i, IN_INFO.fringe_tree_indices[i]);
		    get_root_H(TS_tmp, FT.root_val); // adding valence of H
        FT.root_degree += delta_i;

        // cout << "W2_core i = "<<i<<" FT.root_degree = "<<FT.root_degree <<" FT.root_val = " << FT.root_val <<endl; 

        if (FT.root_val + delta_i <= IN_INFO.atoms[a].VALENCE && FT.root_degree <= 4){
          All_FT_W2_core[a].push_back(FT);
        }
      }
    }

   /* if (delta_i <= IN_INFO.atoms[a].VALENCE){
      FringeTree FT(a, IN_INFO.dmax);
      FT.calc_root_val(IN_INFO.dmax);
      FT.calc_rv(IN_INFO, delta_i);
      FT.root_degree += delta_i;
    ...
    for (size_t i = 0; i < All_FT_W2_core[a].size(); ++i) {
      ...
      if (_DEBUG) {
        cout << "W2_0_core ind = " << ind << " FT.root_charge = " << T.charge[0] << " FT.root_degree = "<<T.root_degree <<" FT.root_val = " << T.root_val <<endl;
        print(T, IN_INFO);
      }

      map_FT_W2_core[make_pair(rsh, ind)].push_back(i);
  ...
  for (unsigned short a = 0; a < M; ++a) {
    ...
    for (size_t i = 0; i < IN_INFO.fringe_tree_indices.size(); ++i){
      ...
      if (IN_INFO.find_index(TS_tmp.delta_2[0]) == a
        ...
        // cout << "W3 i = "<<i<<" FT.root_degree = "<<FT.root_degree <<" FT.root_val = " << FT.root_val <<endl; 

        if (FT.root_val + delta_i <= IN_INFO.atoms[a].VALENCE && FT.root_degree <= 4){
          All_FT_W3[a].push_back(FT);
        }
      }
    }
    ...
    for (size_t i = 0; i < All_FT_W3[a].size(); ++i) {
      ...
      if (_DEBUG) {
        cout << "W3 ind = " << ind << " FT.root_charge = " << T.charge[0] << " FT.root_degree = "<<T.root_degree <<" FT.root_val = " << T.root_val <<endl;
        print(T, IN_INFO);
      }

      map_FT_W3[make_pair(rs, ind)].push_back(i);
    ...
    for (auto& rhps : all_rs_W2) {
      ...
      for (auto& rs : all_rs_W1[l1]) {
        ...
        for (int k = 1; k <= 3; ++k) {
          ...
          if (m1 + k == IN_INFO.atoms[a1].VALENCE && // since we are considering valence with H
            ...
            if (IN_INFO.atom_deg_map.at(ad1) >= IN_INFO.atom_deg_map.at(ad2)){
              e_tmp = rv_edge("int", ad1, ad2, k);
            } else {
              e_tmp = rv_edge("int", ad2, ad1, k);
            }
...
void merge_W3(const input_info& IN_INFO,
              ...
              size_t l_max,  // for tree_component, l_min=l_max = height(T)-k*,
                             // else lmin=1, lmax=
              size_t delta_i = 2, 
				// >= 2, the number of neighbour of the core vertex in core
			bool edge = false
			// to know if given component is v or e component
			// if false then v-component
) {
  size_t M = IN_INFO.num_kind_atoms;
    ...
    for (auto& rs1 : all_rs_W1[l1]) {
      ...
      for (auto& rs3 : all_rs_W3[l2]) {
        ...
        for (int k = 1; k <= 3; ++k) {
          ...
          if ((!edge && m1 + k == IN_INFO.atoms[a1].VALENCE &&
              m2 + k == IN_INFO.target_rs1.val)
				|| (edge && m1 + k == IN_INFO.atoms[a1].VALENCE &&
				m2 + k + delta_i <= IN_INFO.atoms[a2].VALENCE)) 
		  // separately deal with v and e component
		  {
...
void combine_check_tree_component(
    ...
    vector<feasible_pair_W>& all_fp_W, vector<feasible_pair_W>& all_fp_W_loose, size_t l1, size_t l3,
          ...
          for (auto& _w1 : set_w1) {
            ...
            if (TestLessEqAB(w1, IN_INFO.rv1D)) {
              ...
              if (set_w2.find(w2) != set_w2.end()) {
                ...
                // _w1.first << endl;
                all_fp_W.emplace_back(rs3, rs1, l3, _w1.first, l1, ind_w2, k, DAG_direction_status::EXACT_possible);
              }

              if (_LOOSE){
                for (auto& w2 : set_w2){
                  auto _w = SUMinus3D(w1, w2, true);
                  if (TestEqAB_loose_for_fc(_w, IN_INFO.rv1D_loose, IN_INFO) && !TestEqAB(_w, IN_INFO.rv1D)){ // change, 0610
                    size_t ind_w2 = find_trie_index(w2, all_trie_node[l1]);
                    auto& _w2 = map_W1[l1].at(rs1).at(ind_w2);
                    ...
                    all_fp_W.emplace_back(rs3, rs1, l3, _w1.first, l1, ind_w2, k, DAG_direction_status::LOOSE_possible);
                  }
                }
              }
            }
    ...
    if (target_rs == rs3) {
      ...
      for (auto& _w3 : set_w3) {
        ...
        if (TestEqAB(w3, IN_INFO.rv1D)) {
          ...
          // DAG
          _w3.second = true;
        } else if (_LOOSE && TestEqAB_loose_for_fc(w3, IN_INFO.rv1D_loose, IN_INFO)){ // chenge for 0610
          all_fp_W_loose.emplace_back(rs3, l3, _w3.first);
          _w3.second = true;
        }
      }
    }
    ...
    for (auto& rhps : all_rs_W2) {
      ...
      for (auto& rs : all_rs_W1[l1]) {
        ...
        for (int k = 1; k <= 3; ++k) {
          ...
          if (m1 + k == IN_INFO.atoms[a1].VALENCE && // valence check
              m2 + k + 1 <= IN_INFO.atoms[a2].VALENCE) {
            auto& set_left = map_W1[l1].at(rs);
            auto& set_right = map_W2_ind.at(rhps);
                ...
                if (TestLessEqAB(
                  ...
                  if (map_W1[l][_rs].find(ind) != map_W1[l][_rs].end()) {
                    if (map_W1[l][_rs][ind]) {
                      ...
                      map_DAG_W1[tmp].emplace_back(rs, l1, _w1.first, rhps.lrs, _w2,
                                                   k);
                    }
                  }
      ...
      for (auto& rs3 : all_rs_W3[l2]) {
        ...
        for (int k = 1; k <= 3; ++k) {
          ...
          if ((!edge && m1 + k == IN_INFO.atoms[a1].VALENCE &&
              m2 + k == IN_INFO.target_rs1.val)
			|| (edge && m1 + k == IN_INFO.atoms[a1].VALENCE &&
			m2 + k + delta_i <= IN_INFO.atoms[a2].VALENCE)) { // changed for valence
            auto& set_left = map_W1[l1].at(rs1);
            auto& set_right = map_W3[l2].at(rs3);
                ...
                if (TestLessEqAB(new_rv,
                  ...
                  if (map_W3[l][_rs].find(ind) != map_W3[l][_rs].end()) {
                    if (map_W3[l][_rs][ind]) {
                      _w1.second = true;
                      DAG_node tmp(_rs, l, ind);
                      map_DAG_W3[tmp].emplace_back(rs1, l1, _w1.first, rs3,
                                                   _w2.first, k);
                    }
                  }
                }  // if (TestLessEqAB)
                ...
                if (TestLessEqAB(new_rv,
                  ...
                  if (map_W3[l][_rs].find(ind) != map_W3[l][_rs].end()) {
                    if (map_W3[l][_rs][ind]) {
                      _w2.second = true;
                      DAG_node tmp(_rs, l, ind);
                      map_DAG_W3[tmp].emplace_back(rs3, l2, _w2.first, rs1.lrs,
                                                   _w1.first, k);
                    }
                  }
                }  // if (TestLessEqAB)
...
void GenT1(const input_info& IN_INFO,
  ...
  resource_vector null_rv(IN_INFO.num_kind_atom_degree, IN_INFO.num_fc);
  ...
  for (size_t h = min_h; h <= max_h; ++h) {
    if (h == 0) {
      for (auto& rsh : all_rs_W2_core) {
        ...
        root_status rs(rsh.color, rsh.val, rsh.deg);
        if (exact && IN_INFO.target_rs1 != rs){
          continue;
        }
        // cout << "rsh.color = "  << rsh.color << " rsh.deg = " << rsh.deg << "
        // rsh.val = " <<rsh.val << " rsh.height = " << rsh.height << endl;
        for (auto& _w : map_W2_core.at(rsh)) {
          ...
          if (
            (exact && IN_INFO.rv1D != w)
            ||
            (exact && loose && (IN_INFO.rv1D == w || !TestEqAB_loose_for_fc(w, IN_INFO.rv1D_loose, IN_INFO)))
    ...
    for (auto& rhps : all_rs_T2) {
      ...
      for (auto& rs : all_rs_T1[l1]) {
        ...
        for (int k = 1; k <= 3; ++k) {
          ...
          if ((l1 > 0 && m1 + k == IN_INFO.atoms[a1].VALENCE && // changed for valence
               m2 + k + 1 <= IN_INFO.atoms[a2].VALENCE) ||
              (l1 == 0 && k == rs.first.val &&
               m2 + k + 1 <= IN_INFO.atoms[a2].VALENCE)) {
  ...
  for (auto& rsT1 : all_rs_T1[l1]) {
    ...
    for (auto& rsT2 : all_rs_T1[l2]) {
      ...
      for (int k = 1; k <= 3; ++k) {
        ...
        if (((l1 == 0) || (l1 != 0 && m1 + k == IN_INFO.atoms[a1].VALENCE)) &&//valence
            ((l2 == 0) || (l2 != 0 && m2 + k == IN_INFO.atoms[a2].VALENCE)) &&//changed valence
          ...
          for (auto& _w1 : set_w1) {
            ...
            if (TestLessEqAB(w1, IN_INFO.rv1D)) {
              ...
              if (set_w2.find(w2) != set_w2.end()) {
                ...
                all_fp_T.emplace_back(rsT1, rsT2, l1, _w1.first, l2, ind_w2, k, DAG_direction_status::EXACT_possible);
              }

              if (_LOOSE){ // add 0609
                for (auto& w2 : set_w2){
                  auto w = SUMinus3D(w1, w2, true);
                  if (!TestEqAB(w, IN_INFO.rv1D) && TestEqAB_loose_for_fc(w, IN_INFO.rv1D_loose, IN_INFO)){ // change 0610
                    size_t ind_w2 = find_trie_index(w2, all_trie_node_path);
                    auto& _w2 = map_T1[l2].at(rsT2).at(ind_w2);
                    ...
                    all_fp_T.emplace_back(rsT1, rsT2, l1, _w1.first, l2, ind_w2, k, DAG_direction_status::LOOSE_possible);
                  }
                }
              }
            }
    ...
    for (auto& rhps : all_rs_T2) {
      ...
      for (auto& rs : all_rs_T1[l1]) {
        ...
        for (int k = 1; k <= 3; ++k) {
          ...
          if ((l1 > 0 && m1 + k == IN_INFO.atoms[a1].VALENCE && // valence
               m2 + k + 1 <= IN_INFO.atoms[a2].VALENCE) ||// not changed
              (l1 == 0 && k == rs.first.val &&
               m2 + k + 1 <= IN_INFO.atoms[a2].VALENCE)) {
            auto& set_left = map_T1[l1].at(rs);
            auto& set_right = map_T2_ind.at(rhps);
