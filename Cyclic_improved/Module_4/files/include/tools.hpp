    ...
    for (size_t i = 0; i < M; ++i){
        for (size_t j = 0; j <= i; ++j){
            for (size_t k = 1; k <= 3; ++k){
                ...
                cout << "#(" << IN_INFO.atoms[a.color].NAME << a.deg << ", " << IN_INFO.atoms[b.color].NAME << b.deg;
...
void print_rv1D(const input_info& IN_INFO, const resource_vector_1D& rv1D){
	...
	for (size_t i = 1; i < IN_INFO.rv_edges.size(); ++i){
		...
		cout << "#(" << tmp.type << " " << IN_INFO.atoms[tmp.ad1.color].NAME << tmp.ad1.deg << " ";
		cout << IN_INFO.atoms[tmp.ad2.color].NAME << tmp.ad2.deg << " " << tmp.mul << ") = " << rv1D[i] << endl;
	}
}
