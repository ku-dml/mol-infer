/*===========================================================================
  ...
  -----
  Version for the cyclic case

  (Development note)

  *2020/10/01: implementation starts, based on the acyclic case
  *2020/10/06: (v02) when sdf has no H, #H was set to zero, which is mistake

  (Memo)
  - does not identify whether the input is rho-lean 
  ===========================================================================*/
...
#define DEBUG_H 0      // Debug flag for Haraguchi

// CAUTION: The values should NOT be changed!
...
void printFeatureVec(
  ...
  for (size_t i = 0; i < num_instance; ++i) {
    for (size_t j = 0; j < f_value[i].size()-1; ++j) {
      if (j == 0) cout << (int) f_value[i][j] << ",";
      else cout << f_value[i][j] << ",";
    }
    cout << f_value[i][f_value[i].size()-1] << "\n";
  }
}
...
int main(int argc, char **argv){
  ...
  //assignElemOrder(Lambda, elementOrder);
  /* ----- Haraguchi: END ----- */
  ...
  getFeatureVec( G, f_name, f_value,
		 ...
		 Lambda, globalOnePath, globalMoreThanOnePath,
  ...
  */
  ...
  // cerr << "Finished calculating FV4+CYCLIC" << endl;

  // Print the stored values to a given file
  printFeatureVec(num_graphs, f_name, f_value);
