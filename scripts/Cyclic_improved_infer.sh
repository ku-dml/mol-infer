...
echo "E.g.: You have BP_weights.txt and BP_biases.txt,"
echo "      then the prefix should be BP"
...
echo "Default: ${CYCLIC_IMPROVED_DEFAULT_TARGET_VALUE}"
read -e -p "$(echo -e "[${Green}target value${Color_Off}]: ")" TARGET_VALUE
TARGET_VALUE=${TARGET_VALUE:-$CYCLIC_IMPROVED_DEFAULT_TARGET_VALUE}
echo ""

echo "Please supply a chemical specification file (see documents)."
echo "Default: ${CYCLIC_IMPROVED_DEFAULT_SPEC_FILE}"
read -e -p "$(echo -e "[${Green}filename${Color_Off}]: ")" SPEC_FILE
SPEC_FILE=${SPEC_FILENAME:-$CYCLIC_IMPROVED_DEFAULT_SPEC_FILE}
echo ""
...
$PYTHON $MOLINFER_ROOT/Cyclic_improved/Module_3/files/infer_cyclic_graphs_ec_id_vector.py \
    ...
    $MOLINFER_ROOT/Cyclic_improved/bin/fv4_cyclic_stdout
if [ "$?" != "0" ]; then
    echo -e "${Red}"
