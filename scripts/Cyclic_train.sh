...
$MOLINFER_ROOT/Cyclic/bin/$OS/CHECKER "$MOLECULES_FILE"
if [ "$?" != "0" ]; then
    echo -e "${Red}"
...
$PYTHON $MOLINFER_ROOT/Cyclic/bin/eliminate.py \
...
$MOLINFER_ROOT/Cyclic/bin/$OS/FV_ec \
...
$PYTHON $MOLINFER_ROOT/Cyclic/bin/scikit_chemgraph_learning_lim.py \
