 ...
 */
void extendTree(
	const input_info& IN_INFO,         // resource
	FringeTree& T,                     // the FT in build
	const size_t& next_index,          
	const size_t& residual_valence,
	...
	for (Mult m = max_val; m >= 1; --m){
		for (Color b = max_color; b >= 0; --b){
			if (m <= IN_INFO.atoms[b].VALENCE){
				...
				T.root_val = m;
				T.height = 1;
				extendTree(IN_INFO, T, next_index, residual_valence,
						Tau, root_degree);
				Tau.push_back(T);
				T.root_val = 0;
				--T.num_verts;
...
vector <FringeTree> GenAllFringeTrees(
	...
	for (size_t i = 0; i < m; ++i){
		...
		// cout << "num_vert_Ti = " << num_vert_Ti << " AllFringeTrees[i].seq[1].first + root_degree = " << AllFringeTrees[i].seq[1].first + root_degree << endl;
			...
			if (k_i + k_j + root_degree <= IN_INFO.atoms[index_a].VALENCE)
			{
				for (size_t h = j; h < m; ++h){
					...
					if ((k_i + k_j + k_h + root_degree <= IN_INFO.atoms[index_a].VALENCE) &&
						...
						((height_check && (num_vert_Ti >= 3 || num_vert_Tj >= 3 || num_vert_Th >= 3))  || (!height_check)) &&
						(num_vert_Ti > 1 && num_vert_Tj > 1 && num_vert_Th > 1))
					{
						...
						if (AllFringeTrees[j].height > Tijkp.height) Tijkp.height = AllFringeTrees[j].height;
						if (AllFringeTrees[h].height > Tijkp.height) Tijkp.height = AllFringeTrees[h].height;
				...
				if (num_vert_Ti + num_vert_Tj - 1 <= 6 && 
					((height_check && (num_vert_Ti >= 3 || num_vert_Tj >= 3)) || (!height_check))
					&&
					(num_vert_Ti > 1 && num_vert_Tj > 1))
				{
					...
					if (AllFringeTrees[j].height > Tijp.height) Tijp.height = AllFringeTrees[j].height;
...
void GenW1(const input_info& IN_INFO,
	...
	vector <unordered_map <root_status, map <size_t, pair <vector <map_rv>, size_t>>>>& map_W1,
	...
	for (unsigned short a = 0; a < M; ++a){
		...
		for (size_t i = 0; i < All_FT_W1[a].size(); ++i){
			...
			root_status rs(a, T.root_val, T.root_degree);      // here T.root_degree means the real degree, have added the [+2]
			...
			if (map_W1[0][rs].find(ind) == map_W1[0][rs].end()){
				map_W1[0][rs][ind] = make_pair(vector <map_rv> ({map_rv(a, i)}), 1);
			} else {
				if (seq_limit == 0 || map_W1[0][rs][ind].first.size() < seq_limit){
					map_W1[0][rs][ind].first.emplace_back(a, i);
				}
				++map_W1[0][rs][ind].second;
			}
		}
		// if (_DEBUG) cout << "a = " << a <<" All_FT_W1[a].size() = " << All_FT_W1[a].size() << endl;
	...
	for (unsigned short a = 0; a < M; ++a){
		...
		for (size_t i = 0; i < All_FT_W2[a].size(); ++i){
			...
			if (map_W2[hps].find(ind) == map_W2[hps].end()){
				map_W2[hps][ind] = make_pair(vector <map_rv> ({map_rv(a, i)}), 1);
			} else {
				if (seq_limit == 0 || map_W2[hps][ind].first.size() < seq_limit){
					map_W2[hps][ind].first.emplace_back(a, i);
				}
				++map_W2[hps][ind].second;
			}
		}
		// if (_DEBUG) cout << "a = " << a <<" All_FT_W2[a].size() = " << All_FT_W2[a].size() << endl;
	}
...
void GenW2_0_core(const input_info& IN_INFO,
	...
	for (unsigned short a = 0; a < M; ++a){
		...
		for (size_t i = 0; i < All_FT_W2_core[a].size(); ++i){
			...
			root_status_height rsh(a, T.root_val, T.root_degree, T.height);      // here T.root_degree means the real degree, have added the [+2]
			all_rs_W2_core.emplace(rsh);
			// cout << "rsh.color = "  << rsh.color << " rsh.deg = " << rsh.deg << " rsh.val = " <<rsh.val << " rsh.height = " << rsh.height << endl;
			...
			if (map_W2_core[rsh].find(ind) == map_W2_core[rsh].end()){
				map_W2_core[rsh][ind] = make_pair(vector <map_rv> ({map_rv(a, i)}), 1);
			} else {
				if (seq_limit == 0 || map_W2_core[rsh][ind].first.size() < seq_limit){
					map_W2_core[rsh][ind].first.emplace_back(a, i);
				}
				++map_W2_core[rsh][ind].second;
			}
		}
		// if (_DEBUG) cout << "a = " << a << " M = " << M << " All_FT_W2_core[a].size() = " << All_FT_W2_core[a].size() << endl;
	}
...
void GenW3_0(const input_info& IN_INFO,
	...
	vector <unordered_map <root_status, map <size_t, pair <vector <map_rv>, size_t>>>>& map_W3,
	...
	for (unsigned short a = 0; a < M; ++a){
		...
		for (size_t i = 0; i < All_FT_W3[a].size(); ++i){
			...
			if (map_W3[0][rs].find(ind) == map_W3[0][rs].end()){
				map_W3[0][rs][ind] = make_pair(vector <map_rv> ({map_rv(a, i)}), 1);
			} else {
				if (seq_limit == 0 || map_W3[0][rs][ind].first.size() < seq_limit){
					map_W3[0][rs][ind].first.emplace_back(a, i);
				}
				++map_W3[0][rs][ind].second;
			}
		}
		// if (_DEBUG) cout << "a = " << a <<" All_FT_W3[a].size() = " << All_FT_W3[a].size() << endl;
	}
...
void merge_W1(const input_info& IN_INFO,
	...
	for  (size_t l = 1; l <= len; ++l){
							...
							for (auto& _w1 : set_left){
								...
								if (
									...
									if (map_W1[l][_rs].find(ind) == map_W1[l][_rs].end()){
										...
										vector <map_rv> tmp;
										tmp.clear();
										map_W1[l][_rs][ind] = make_pair(tmp, 0);
									}
		...
		if (_DEBUG) cout << "end subtrees l = " << l << " len = " << len << " vector_size = " << vector_size << " time = " << globalTimeKeeper::tt.diff() - _start_time << endl;
...
void merge_W3(const input_info& IN_INFO,
	...
	vector_2D <PathTrie>& all_trie_node,
	size_t l_min,   //
	size_t l_max,	// for tree_component, l_min=l_max = height(T)-k*, else lmin=1, lmax=
	size_t delta_i = 2  // >= 2, the number of neighbour of the core vertex in core
){
	size_t M = IN_INFO.num_kind_atoms;
	...
	for (size_t l = l_min; l <= l_max; ++l){
		...
		for (auto& rs1 : all_rs_W1[l1]){
			...
			for (auto& rs3 : all_rs_W3[l2]){
				...
				for (size_t k = 1; k <= 3; ++k){
					...
					// if (DEBUG) cout << "a1 = " << a1 << " d1 = " << d1 << " m1 = " << m1 << " a2 = " << a2 << " d2 = " << d2 << " m2 = " << m2 << " k = " << k << endl;
					...
					if (
						...
						for (auto& _w1 : set_left){
							...
							for (auto& _w2 : set_right){
								...
								if (
									TestLessEqAB(new_rv, IN_INFO.rv1D)    // w1 + w2 + 1_gamma <= x_star
								){
									root_status _rs(rs3);
									_rs.val += k;
									...
									if (map_W3[l][_rs].find(ind) == map_W3[l][_rs].end()){
										...
										vector <map_rv> tmp;
										tmp.clear();
										map_W3[l][_rs][ind] = make_pair(tmp, 0);
									}
		...
		if (_DEBUG) cout << "rooted core-subtrees l = " << l << " l_min = " << l_min << " l_max = " << l_max << " vector_size = " << vector_size << " time = " << globalTimeKeeper::tt.diff() - _start_time << endl;
	}		
}

void merge_W3_tree(const input_info& IN_INFO,
	unordered_set <half_path_status>& all_rs_W2,
	vector <unordered_set <root_status>>& all_rs_W3,
	unordered_map <half_path_status, map <size_t, pair <vector <map_rv>, size_t>>>& map_W2,
	vector <unordered_map <root_status, map <size_t, pair <vector <map_rv>, size_t>>>>& map_W3,
	...
	for (size_t l = l_min; l <= l_max; ++l){
		...
		// cout << "l = "  << l << " l_min = " << l_min << " l_max = " << l_max  <<  endl;
		...
		for (auto& rs1 : all_rs_W2){
			...
			for (auto& rs3 : all_rs_W3[l2]){
				...
				for (size_t k = 1; k <= 3; ++k){
					...
					// if (_DEBUG) cout << "a1 = " << a1 << " d1 = " << d1 << " m1 = " << m1 << " a2 = " << a2 << " d2 = " << d2 << " m2 = " << m2 << " k = " << k << endl;

					if (
						((l2 > 0) &&
						m1 + k + 1 <= IN_INFO.atoms[a1].VALENCE
						&&
						...
						// if (_DEBUG) cout << "A a1 = " << a1 << " d1 = " << d1 << " m1 = " << m1 << " a2 = " << a2 << " d2 = " << d2 << " m2 = " << m2 << " k = " << k << endl;
						rv_edge e_tmp("in", ad2, ad1, k);  // direction: (a2,d2)->(a1,d1)
						size_t e_tmp_ind = IN_INFO.rv_edge_map.at(e_tmp);
...
void combine_check_tree_component(const input_info& IN_INFO,
	vector <unordered_set <root_status>>& all_rs_W1,
	vector <unordered_set <root_status>>& all_rs_W3,
	vector <unordered_map <root_status, map <size_t, pair <vector <map_rv>, size_t>>>>& map_W1,
	vector <unordered_map <root_status, map <size_t, pair <vector <map_rv>, size_t>>>>& map_W3,
	...
	for (auto& rs3 : all_rs_W3[l3]){
		...
		for (auto& rs1 : all_rs_W1[l1]){
			...
			for (size_t k = 1; k <= 3; ++k){
				...
				if (m1 + k <= IN_INFO.atoms[a1].VALENCE && m2 + k <= IN_INFO.atoms[a2].VALENCE 
					&&
					...
					for (auto& _rv : _set_w2){
						auto _w2 = traverse(_rv.first, all_trie_node[l1], IN_INFO.rv1D_size);
						set_w2.insert(_w2);
					}
								...
								for (auto& seq1 : seq1_vector){
									for (auto& seq2 : seq2_vector){
										if (pair_limit == 0 || count <= pair_limit){
											all_seq_tree.emplace_back(seq1, seq2, k, false);
										}	
									} // for seq2
								}// for seq1	
							}
	...
	// if (_DEBUG) cout << "Number of feasible pairs (h = core_height) = " << count_h << endl;
	// if (_DEBUG) cout << "Number of feasible pairs (others) = " << count - count_h << endl;
...
void GenT1(const input_info& IN_INFO,
	...
	vector <unordered_map <root_status_T, map <size_t, pair <vector <map_tree>, size_t>>>>& map_T1,
			...
			for (auto& rsh : all_rs_W2_core){
				...
				for (auto& _w : map_W2_core.at(rsh)){
					...
					if (map_T2[rsh].find(ind) == map_T2[rsh].end()){
						...
						map_T2[rsh][ind] = make_pair(tmp, _w.second.second);
					} else {
						for (auto& _seq: _w.second.first){
							if (seq_limit == 0 || map_T2[rsh][ind].first.size() < seq_limit){
								map_T2[rsh][ind].first.emplace_back(_seq, rsh.height);
							}
						}
...
void merge_T1(const input_info& IN_INFO,
	...
	for (size_t l = 1; l <= len; ++l){
			...
			for (auto& rs : all_rs_T1[l1]){
				...
				for (size_t k = 1; k <= 3; ++k){
					...
					if (
						...
						m2 + k + 1 <= IN_INFO.atoms[a2].VALENCE)
						||
						(l1 == 0 && k == rs.first.val && m2 + k + 1 <= IN_INFO.atoms[a2].VALENCE)
					){
						...
						while  (ind != start_index || flag){
							...
							// if (_DEBUG) cout << "ind = " <<ind << " index_size = " <<  index_size  << endl;
							++ind;
							if (ind == index_size){
								ind = 0;
							}
							...
							for (auto& _w1 : set_left){
								...
								auto w1 = traverse(_w1.first, all_trie_node_path, IN_INFO.rv1D_size);
								auto new_rv = SUMinus3D(w1, w2, true);
								...
								if (
									...
									if (map_T1[l][_rsT].find(ind) == map_T1[l][_rsT].end()){
										...
										vector <map_tree> tmp;
										tmp.clear();
										map_T1[l][_rsT][ind] = make_pair(tmp, 0);
									}
									...
									for (auto& seq1 : seq1_vector){
										for (auto& seq2 : seq2_vector){
											if (seq_limit == 0 || _seq.size() < seq_limit){
												_seq.emplace_back(seq1, seq2, k, h, true);
											}
										} // for seq2
									}// for seq1	
									_num += _w1.second.second * _w2_second.second;					
		...
		if (_DEBUG)  cout << "bi-rooted core-subtrees l = " << l << " len = " << len << " vector_size = " << vector_size << " time = " << globalTimeKeeper::tt.diff() - _start_time << endl;
		if (_EXP) bi_rooted_core_subtrees_sum += vector_size;
	}// for l
}

// A function to find feasible pairs
void combine_check_edge_component(const input_info& IN_INFO,
	vector <unordered_set <root_status_T>>& all_rs_T1,
	vector <unordered_map <root_status_T, map <size_t, pair <vector <map_tree>, size_t>>>>& map_T1,
	vector <PathTrie>& all_trie_node_path,
	...
	size_t l1,
	size_t l2,
	root_status& target_rs1,
	root_status& target_rs2,
	const size_t& core_height,
	size_t& count_num_h,
	size_t& count_num
){
		...
		for (auto& rsT2 : all_rs_T1[l2]){
			...
			for (size_t k = 1; k <= 3; ++k){
				...
				if (
					...
					// if (_DEBUG) cout << "rsT1.first.color = " << rsT1.first.color << " rsT1.first.deg = " << rsT1.first.deg << " rsT1.first.val = " << rsT1.first.val << endl;
					// if (_DEBUG) cout << "rsT2.first.color = " << rsT2.first.color << " rsT2.first.deg = " << rsT2.first.deg << " rsT2.first.val = " << rsT2.first.val << endl;
				
					// if (_DEBUG) cout << "a1 = " << a1 << " d1 = " << d1 << " m1 = " << m1 << " h1 = " << h1 << endl;
					// if (_DEBUG) cout << "a2 = " << a2 << " d2 = " << d2 << " m2 = " << m2 << " h2 = "  << h2 <<" k = " << k << endl;
					...
					set <resource_vector_1D> set_w2;
					for (auto& _rv : _set_w2){
						auto _w2 = traverse(_rv.first, all_trie_node_path, IN_INFO.rv1D_size);
						set_w2.insert(_w2);
					}
							...
							if (set_w2.find(w2) != set_w2.end()){
								...
								for (auto& seq1 : seq1_vector){
									for (auto& seq2 : seq2_vector){
										if (pair_limit == 0 || count <= pair_limit){
											if (h == core_height){
												all_seq_h.emplace_back(seq1, seq2, k, h, false);
											} else {
