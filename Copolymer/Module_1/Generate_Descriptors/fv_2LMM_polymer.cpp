/************************************************************
  ...
  Output more information about fv
  ...
  Old version: 2LM-M_polymer_v05.1
...
int main(int argc, char *argv[]){
  ...
  if(need_electric_density){
    ...
    if(Par.test_use)
       add_electric_density(Train_without_H, test_val);
  }
  ...
  cout << "NUM_DESCRIPTORS:\t" << ftr_name.size() << endl;
  cout << "NUM_Gamma_IN:\t" << EC_in.size() << endl;
  cout << "NUM_Gamma_link:\t" << EC_link_edge.size() << endl;
  cout << "NUM_Gamma_in_plus_link:\t" << EC_in.size() + EC_link_edge.size() << endl;
  ...
  for(auto iter = Lambda_in.begin(); iter != Lambda_in.end(); ++iter){
    Lambda.emplace(iter->first, iter->second);
  }
  for(auto iter = Lambda_ex.begin(); iter != Lambda_ex.end(); ++iter){
    Lambda.emplace(iter->first, iter->second);
  }

  Lambda.emplace("H1", 0);

  cout << "Atom-Valance:" << endl;
  for(auto iter = Lambda.begin(); iter != Lambda.end(); ++iter){
    cout << iter->first << "\t";
  }
