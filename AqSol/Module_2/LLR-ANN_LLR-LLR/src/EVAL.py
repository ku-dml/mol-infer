...
from src import LR, ALR, ANN, RF, DT, SVM, KSVM
from src import CLSF, FUN

import pulp 

import time,sys,copy,itertools,math,warnings,traceback
...
FORBIDDEN_WORD = ["way", "selected_fv_filename", "metric"]
...
#                             f.write(f"{_w} ")
#                         f.write(f"\n{lr.intercept_}\n")
#                 elif params_D1["way"] == "BSP" or params_D1["way"] == "MLR":
#                     output_filename = output_prefix_D1 + f"{split_seed}_{ff}_linreg.txt"
...
#                             f.write(f"{_w} ")
#                         f.write(f"\n{lr.intercept_}\n")
#                 elif params_D2["way"] == "BSP" or params_D2["way"] == "MLR":
#                     output_filename = output_prefix_D2 + f"{split_seed}_{ff}_linreg.txt"
...
#                 with open(log_filename_D1, 'a') as f:
#                     f.write(f"{split_seed}\t{r2train}\t{r2test}\t{time_tmp}\n")
#                     f.write(f"{len(train_D1)}\t{len(train_D2)}\t{len(test_D1)}\t{len(test_D2)}\n")
#                     f.write(f"{r2_score(y_train_D1, y_predict_train_D1)}\t{r2_score(y_test_D1, y_predict_test_D1)}\n")
#                 with open(log_filename_D2, 'a') as f:
#                     f.write(f"{split_seed}\t{r2train}\t{r2test}\t{time_tmp}\n")
#                     f.write(f"{len(train_D1)}\t{len(train_D2)}\t{len(test_D1)}\t{len(test_D2)}\n")
#                     f.write(f"{r2_score(y_train_D2, y_predict_train_D2)}\t{r2_score(y_test_D2, y_predict_test_D2)}\n")
...
def eval_single(x, y, fv_list, log_filename, params, output_prefix=None, error_check=False, Sbar=FUN.Status_Bar()):
    R2_train = []
    R2_test = [] 
    ...
    Sbar.set_total_times(Times_eva * Fold)
    Sbar.output()

    while seed_success < Times_eva:
        kf = KFold(n_splits=Fold, shuffle=True, random_state=split_seed)

        Sbar.set_present(seed_success * Fold)
        ...
        try:
            ...
            for train, test in kf.split(x):
                ...
                time_start = time.time()
                if params["way"] == "Lasso":
                    lr, y_predict_train, y_predict_test, _ = \
                        LR.learn_LLR_eval(x_train, y_train, x_test, y_test, a=params["lambda"])
                    if output_prefix is not None:
                        output_filename = output_prefix + f"_{split_seed}_{ff}.txt"
                        LR.write_file(x_sel.shape[1], lr, output_filename)                   
                elif params["way"] == "BSP" or params["way"] == "MLR":
                    lr, y_predict_train, y_predict_test = \
                        LR.learn_MLR_eval(x_train, y_train, x_test, y_test)
                    if output_prefix is not None:
                        output_filename = output_prefix + f"_{split_seed}_{ff}.txt"
                        LR.write_file(x_sel.shape[1], lr, output_filename)   
                elif params["way"] == "ANN":
                    ann, y_predict_train, y_predict_test = \
                        ANN.learn_ANN_eval(x_train, y_train, x_test, y_test, 
                            arch=params["arch"], rho=params["rho"], maxitr=params["maxitr"])
                    if output_prefix is not None:
                        output_filename = output_prefix + f"_{split_seed}_{ff}"
                        ANN.write_weights_biases(ann[0], output_filename)
                elif params["way"] == "RF":
                    params_for_RF = {k: v for k, v in params.items() if k not in FORBIDDEN_WORD}
                    rf, _, _, y_predict_train, y_predict_test = \
                        RF.learn_random_forest(x_train, y_train, x_test, y_test, params=params_for_RF)
                    if output_prefix is not None:
                        output_filename = output_prefix + f"_{split_seed}_{ff}.txt"
                        RF.write_file(rf, output_filename)
                elif params["way"] == "ALR":
                    ...
                    _, r2train, r2test, _ = \
                        ALR.learn_ALR_pre(x_train, y_train, x_test, y_test, params["lambda"], 
                            epsilon_result, alpha_result, selected, I_minus, I_plus, metric=params["metric"])
                elif params["way"] == "DT":
                    params_for_DT = {k: v for k, v in params.items() if k not in FORBIDDEN_WORD}
                    _, _, y_predict_train, y_predict_test = \
                        DT.learn_decision_tree(x_train, y_train, x_test, y_test, params=params_for_DT)
                elif params["way"] == "SVM":
                    params_for_SVM = {k: v for k, v in params.items() if k not in FORBIDDEN_WORD}
                    _, _, y_predict_train, y_predict_test = \
                        SVM.learn_SVM(x_train, y_train, x_test, y_test, params=params_for_SVM)
                elif params["way"] == "KSVM":
                    params_for_KSVM = {k: v for k, v in params.items() if k not in FORBIDDEN_WORD}
                    _, _, y_predict_train, y_predict_test = \
                        KSVM.learn_SVM(x_train, y_train, x_test, y_test, params=params_for_KSVM)
                time_end = time.time()
                time_D1 = time_end - time_start
                sum_time += time_D1
                ...
                if params["way"] != "ALR":
                    if params["metric"] == 'R2':
                        r2train = r2_score(y_train, y_predict_train)
                        r2test = r2_score(y_test, y_predict_test)
                    elif params["metric"] == 'BACC':
                        r2train, r2test = CLSF.calc_bacc(None, x_train, y_train, x_test, y_test, y_predict_train, y_predict_test)
                    elif params["metric"] == 'AUCROC':
                        r2train, r2test = CLSF.calc_aucroc(None, x_train, y_train, x_test, y_test, y_predict_train, y_predict_test)
