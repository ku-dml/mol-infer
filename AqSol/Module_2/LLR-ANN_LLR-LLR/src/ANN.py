...
def LLR_ANN_Lasso_pre(
    x, y, log_filename, quad=True, metric='R2', Sbar=FUN.Status_Bar()
):
    if quad:
    ...
    best_lambda, _, _ = LR.Lasso_pre(x, y, log_filename, lambda_list, STEP, metric=metric, Sbar=Sbar)
...
def learn_ANN_pre(x_train, y_train, x_test, y_test, arch, maxitr, metric='R2'):
    ...
    for t in range(10, MaxItr+1, 10):
        ...
        if metric == 'R2':
            ...
            r2train, r2test = CLSF.calc_bacc(reg, x_train, y_train, x_test, y_test)
        elif metric == 'AUCROC':
            r2train, r2test = CLSF.calc_aucroc(reg, x_train, y_train, x_test, y_test)
        R.append((t,r2train, r2test))

    return R

def LLR_ANN_pre(x, y, fv_list, log_filename, quad=True, metric='R2', Sbar=FUN.Status_Bar()):
    ...
    best_lambda = LLR_ANN_Lasso_pre(x, y, log_filename, quad, metric=metric, Sbar=Sbar_pre)
    ...
    Sbar.set_total_times(len(Arch) * Times_pre * Fold)
    Sbar.output()
    ...
    for arch in Arch:
        ...
        for split_seed in range(1, Times_pre+1):
            ...
            for train, test in kf.split(xsel):
                fold += 1
                start_time = time.time()
                R = learn_ANN_pre(xsel[train], y[train], xsel[test], y[test], arch, MaxItr, metric=metric)
