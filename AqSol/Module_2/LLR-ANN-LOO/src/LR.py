...
def Lasso_pre(x, y, log_filename, lambda_list=lambda_list, STEP=STEP):
    ...
    for lmd in lambda_list:
        ...
        for i in range(n):
            ...
            train.remove(i)
            test = [i]
            _, nonzero, r2train, y_test_predict = learn_Lasso_pre(x[train], y[train], x[test], y[test], a=lmd)
            time_end = time.time()
            # TsR[lmd].append(r2test)
            r2test.append(y_test_predict)
            TrR[lmd].append(r2train)
            with open(log_filename, 'a') as f:
                f.write(f"{lmd}, {i}, {r2train}, {r2test[-1]}, {nonzero}, {time_end - time_start}\n")
                f.close()  

        TsR[lmd] = r2_score(y, r2test)
    ...
    best_lambda = max(TsR, key=TsR.get)
