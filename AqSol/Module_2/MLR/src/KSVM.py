...
C_list = list(np.arange(0.0001, 0.001, 0.0002)) + list(np.arange(0.001, 0.01, 0.002)) + \
         list(np.arange(0.01, 0.1, 0.02)) + list(np.arange(0.1, 1.0, 0.1)) + \
...
####################################################
def learn_SVM(
    x_train, y_train, x_test, y_test, params=SVR().get_params(), metric='R2'
):
    if metric == 'R2':
        svm = SVR()
