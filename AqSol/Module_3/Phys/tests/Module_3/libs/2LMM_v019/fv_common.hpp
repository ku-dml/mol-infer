...
AtomBond2Int init_convalent_radius(){
  AtomBond2Int CR;
  // Row 2
  CR["B"][1]=85;    CR["C"][2]=78;    CR["C"][3]=73;
  CR["C"][1]=75;    CR["C"][2]=67;    CR["C"][3]=60;
  CR["N"][1]=71;    CR["N"][2]=60;    CR["N"][3]=54;
  CR["O"][1]=63;    CR["O"][2]=57;    CR["O"][3]=53;
  CR["F"][1]=64;    CR["F"][2]=59;    CR["F"][3]=53;

  // Row 3
  CR["Si"][1]=116;  CR["Si"][2]=107;  CR["Si"][3]=102;  
  CR["P"][1]=111;   CR["P"][2]=102;   CR["P"][3]=94;
  CR["S"][1]=103;   CR["S"][2]=94;    CR["S"][3]=95;
  CR["Cl"][1]=99;   CR["Cl"][2]=95;   CR["Cl"][3]=93;

  // Row 4
  CR["As"][1]=121;  CR["As"][2]=114;  CR["As"][3]=106;
  CR["Se"][1]=116;  CR["Se"][2]=107;  CR["Se"][3]=107;
  CR["Br"][1]=114;  CR["Br"][2]=109;  CR["Br"][3]=110;

  // Row 5
  CR["I"][1]=133;   CR["I"][2]=129;   CR["I"][3]=125;
  return CR;
}
...
/***** get atoms that are used as "R" in option R *****/
...
void get_opt_R(const AtomBond2Int& CR, const Atom2Int M, 
  ...
  for(int k=0;k<num_graphs;k++){
    ...
    for(auto itr=M.begin();itr!=M.end();itr++){
      freq[itr->first] = 0;
      eledeg[itr->first] = 0.0;
      bl[itr->first] = 0.0;
      vec.push_back(0.0);
      vec.push_back(0.0);
    }    
    for(auto v:GS[k].V){
      if(v.ht!=2 || M.find(v.atom) == M.end())
	continue;
      for(auto e:v.edge){
	VertexStruct u = GS[k].V[e.v];
	if(u.ht==1 && u.atom!="C"){
	  freq[v.atom]++;
	  eledeg[v.atom] += v.eledeg;
	  if(CR.find(v.atom) == CR.end()){
	    cerr << "error: convalent radius of " << v.atom << " is not registered." << endl;
	    exit(1);
	  }
	  if(CR.find(v.atom) == CR.end()){
	    cerr << "error: convalent radius of " << u.atom << " is not registered." << endl;
	    exit(1);
	  }
	  bl[v.atom] += (CR.at(v.atom)).at(e.bond)
	    + (CR.at(u.atom)).at(e.bond);
	}
      }
    }
    ...
    for(auto itr=M.begin();itr!=M.end();itr++){
      int j = itr->second;
      if(freq[itr->first]>0){
	vec[2*j] = eledeg[itr->first] / freq[itr->first];
	vec[2*j+1] = bl[itr->first] / freq[itr->first];
      }
    }
    for(int j=0;j<vec.size();j++)
      ftr_val[k].push_back(vec[j]);
  }
