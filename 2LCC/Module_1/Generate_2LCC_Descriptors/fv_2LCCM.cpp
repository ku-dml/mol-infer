...
int main(int argc, char *argv[]){
  ...
  /**** read input.sdf *****/
  Train_with_H = read_sdf(Par.sdf_file);//直接读SDF返回的图集合
  ...
  // 13. dcp_i(G): frequency of CC (circle-configurations) in circle
  vector<string> cc_ftr_name;
  vector<vector<double>> cc_ftr_val;
  vector<vector<double>> cc_ftr_test_val;
  
  Circle_Configuration(Train_with_H,Mass,cc_ftr_name,cc_ftr_val);
  if(Par.test_use)
    Circle_Configuration_for_test(Test_with_H,Mass,cc_ftr_name,cc_ftr_test_val);
  for (auto& name:cc_ftr_name) {
      ftr_name.emplace_back(name);
  }
    for (int i = 0; i < cc_ftr_val.size(); ++i) {
        for (int j = 0; j < cc_ftr_val[0].size(); ++j) {
            ftr_val[i].emplace_back(cc_ftr_val[i][j]);
        }
    }
  if(Par.test_use){
    for(int k=0; k<Test_with_H.size(); k++){
      for (auto x:cc_ftr_test_val[k]){
        test_val[k].push_back(x);
      }
    }
  }
  ...
  cout << "NUM_CC\t" << cc_ftr_name.size() <<endl;
