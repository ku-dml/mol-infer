/************************************************************
  ...
  (v018) rank is implemented
    (v0181) the number of descriptors is output.
  (v019) atom charge and bond-length are computed by option. 
...
/********** read arguments **********/
void output_usage(int argc, char *argv[]){
  fprintf(stderr, "usage: %s [option:-H](input.sdf)(OUTPUT)[(test.sdf)(TEST)]\n\n", argv[0]);
  fprintf(stderr, "Caution: in this model, input.sdf MUST BE completely H-defined.\n\n");
  /***/
  fprintf(stderr, "Options:\n");
  fprintf(stderr, "\t-R ... atom charge and bond-length\n");
  /***/
  fprintf(stderr, "\nThe program outputs:\n");
...
void read_arguments(int argc, char *argv[], Param& Par){
  ...
  if(argc!=s+2 && argc!=s+4){
    output_usage(argc, argv);
    exit(1);
  }
  
  Par.sdf_file = argv[s];
  ost << argv[s+1] << "_desc.csv";
  Par.csv_file = ost.str();
  ost.str("");
  ost.clear();
  
  ost << argv[s+1] << "_desc_norm.csv";
  Par.norm_csv_file = ost.str();
  ost.str("");
  ost.clear();
  
  ost << argv[s+1] << "_fringe.txt";
  ...
  if(argc==s+2){
    ...
    Par.test_use = true;
    Par.test_sdf = argv[s+2];
    ost << argv[s+3] << "_desc.csv";
    Par.test_csv = ost.str();
    ost.str("");
    ost.clear();
    ost << argv[s+3] << "_desc_norm.csv";
...
int main(int argc, char *argv[]){
  ...
  Mass = init_mass();
  // initialize atom-bond to convalent radius
  CR = init_convalent_radius();
  ...
  for(int k = 0;k<Train_without_H.size(); k++)
    Train_without_H[k] = compute_ht_in_ex(Train_without_H[k], false);  
  ...
  if(Par.test_use){
    ...
    for(int k=0;k<Test_without_H.size();k++)
      Test_without_H[k] = compute_ht_in_ex(Test_without_H[k], false);  
  }
  ...
  /***** options *****/
  // option R: atom charge and bond-lendth [Hunt et al., 2020]
  if(Par.opt_R){
    vector < ChemGraph > Tr;
    Atom2Int opt_R_atom;
    Tr = read_sdf(Par.sdf_file);
    for(int k=0;k<Tr.size(); k++)
      Tr[k] = compute_ht_in_ex(Tr[k], true);
    opt_R_atom = get_exist_opt_R(Tr);
    for(auto itr=opt_R_atom.begin();itr!=opt_R_atom.end();itr++){
      ftr_name.push_back("OptR_AC_"+itr->first);
      ftr_name.push_back("OptR_BL_"+itr->first);
    }
    get_opt_R(CR, opt_R_atom, Tr, ftr_val);
    
    if(Par.test_use){
      vector < ChemGraph > Ts;
      Ts = read_sdf(Par.test_sdf);
      for(int k=0;k<Ts.size(); k++)
	Ts[k] = compute_ht_in_ex(Ts[k], true); 
      get_opt_R(CR, opt_R_atom, Ts, test_val);      
    }
  }
