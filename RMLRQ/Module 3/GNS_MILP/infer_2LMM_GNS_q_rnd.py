...
#   2022/09/03   the random linear functions
######
...
RANDOM_SEED_LF = 100 # random seed for the random linear functions
####################################################
...
#### The next part is to implement grid search procedure ### 
    # new random linear functions, 0903
    ...
    for theta in range(p_max):
        ...
        # THETA_x_hat[theta], _ = prepare_variables_nor_std_fv(K_l, f"{theta_counter}")
        # MILP = add_constraints_nor_std_fv(
        #     MILP,
        ...
        THETA_LR[theta] = lr_q_inverter.create_random_LRq(LR, RANDOM_SEED_LF + theta)
        # print(THETA_LR[theta].coef, THETA_LR[theta].bias, THETA_LR[theta].K_q_ind)
        ...
        #     f"{theta_counter}")      
        MILP = add_constraints__LRq(MILP, x_hat, descriptors_q_x, descriptors_q_y, 
            descriptors_q_x_minus, descriptors_q_y_minus, 
            K_l, K_q, K_q_ind, THETA_LR[theta], mass_ind, mass_n, forbidden_node,
            f"{theta_counter}")        
    ...
    for theta in range(p_max):
        ...
        THETA_LR[theta].build_constraints(MILP, 
                    descriptors_q_x, descriptors_q_y, 
                    descriptors_q_x_minus, descriptors_q_y_minus,
                    f"{theta_counter}")
    ...
    if pulp.LpStatus[MILP.status] == "Optimal":
        ...
        for theta in range(p_max):
            ...
            #     THETA_descriptors_q_x_minus[theta], THETA_descriptors_q_y_minus[theta])
            ALPHA_star[theta] = THETA_LR[theta]._predict_alpha_star(x_hat, 
                descriptors_list, descriptors_q_x, descriptors_q_y,
                descriptors_q_x_minus, descriptors_q_y_minus)
        print(ALPHA_star)
        ...
        for z in Z_sort:
            ...
            for i in range(p_max):
                print(ALPHA_star[i] + (z[i] - 0.5) * Vecdelta[i], ALPHA_star[i] + (z[i] + 0.5) * Vecdelta[i])
                THETA_LR[i].build_constraints_y(MILPp, ALPHA_star[i] + (z[i] - 0.5) * Vecdelta[i], 
                    ALPHA_star[i] + (z[i] + 0.5) * Vecdelta[i],
                    f"{i+1}")
            ...
            if pulp.LpStatus[MILPp.status] == "Optimal":
                ...
                for theta in range(p_max):
                    ...
                    #     THETA_descriptors_list[theta], THETA_descriptors_q_x[theta], THETA_descriptors_q_y[theta],
                    #     THETA_descriptors_q_x_minus[theta], THETA_descriptors_q_y_minus[theta])
                    ALPHA_star_tmp[theta] = THETA_LR[theta]._predict_alpha_star(x_hat,
                        descriptors_list, descriptors_q_x, descriptors_q_y,
                        descriptors_q_x_minus, descriptors_q_y_minus)
